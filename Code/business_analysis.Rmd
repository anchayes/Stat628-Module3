...
mostfreq1 %>% ggplot(aes(Word, Freq)) + geom_col() + xlab(NULL) + coord_flip()
```
...
#make a copy and remove punctuation 
copygood <- good 
copygood$attributes <- gsub('[[:punct:] ]+',' ',copygood$attributes)
#make frequency matrix of terms 
datgood <- as.matrix(TermDocumentMatrix(copygood$attributes))
FreqMatgood <- data.frame(Word= rownames(datgood), 
                      Freq = rowSums(datgood), 
                      row.names = NULL)
FreqMatgood[order(FreqMatgood[,2],decreasing = TRUE),]
#remove false, true,  none 
copyfreqgood <- FreqMatgood
copyfreqgood <- copyfreqgood[-which(copyfreqgood$Word== "false"),]
copyfreqgood <- copyfreqgood[-which(copyfreqgood$Word== "true"),]
copyfreqgood <- copyfreqgood[-which(copyfreqgood$Word== "none"),]
...
```{r}
#plot most frequent attributes listed in restaurant 
mostfreqgood %>% ggplot(aes(Word, Freq)) + geom_col() + xlab(NULL) + coord_flip()
```
...
#remove punctuation 
copybad <- bad 
copybad$attributes <- gsub('[[:punct:] ]+',' ',copybad$attributes)
#make frequency matrix of terms 
datbad <- as.matrix(TermDocumentMatrix(copybad$attributes))
FreqMatbad <- data.frame(Word= rownames(datbad), 
                      Freq = rowSums(datbad), 
                      row.names = NULL)
FreqMatbad[order(FreqMatbad[,2],decreasing = TRUE),]
#remove false, true,  none 
copyfreqbad <- FreqMatbad
copyfreqbad <- copyfreqbad[-which(copyfreqbad$Word== "false"),]
copyfreqbad <- copyfreqbad[-which(copyfreqbad$Word== "true"),]
copyfreqbad <- copyfreqbad[-which(copyfreqbad$Word== "none"),]
...
mostfreqbad %>% ggplot(aes(Word, Freq)) + geom_col() + xlab(NULL) + coord_flip()
```
...
hist(seating$z_score)
hist(noseating$z_score)
...
hist(takeout$z_score)
hist(notakeout$z_score)
